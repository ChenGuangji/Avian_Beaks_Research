...
calculate_pgls_residuals <- function(tree, df, label = NULL) {
  ...
  # 进行PGLS拟合
  model <- pgls(log10(beak_depth) ~ log10(mass), data = stats, lambda = "ML")
  # 提取残差
...
df <- filtered_data %>% dplyr::select(Species,pgls.group,mass,beak_depth)
df <- na.omit(df)
...
p1 <- ggplot() +
  ...
  geom_point(data = group_points_4,
             aes(x = log10(mass), y = log10(beak_depth)),
