...
calculate_pgls_residuals <- function(tree, df, label = NULL) {
  ...
  # 进行PGLS拟合
  model <- pgls(log10(beak_length) ~ log10(mass), data = stats, lambda = "ML")
  # 提取残差
...
df <- filtered_data %>% dplyr::select(Species,pgls.group,mass,beak_length)
df <- na.omit(df)
...
p1 <- ggplot() +
  ...
  geom_point(data = group_points_1,
             aes(x = log10(mass), y = log10(beak_length)),
